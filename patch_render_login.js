...
const eIdx = html.indexOf('function sendOtp(){');
...
html = html.substring(0, sIdx) + newLoginFn + html.substring(eIdx + 'async function loginPlantChanged(v){'.length);
// wait, loginPlantChanged is completely replaced, so we need to snip out the original loginPlantChanged body.
// We'll use a regex or string replacement to cut out the old one correctly.

fs.writeFileSync('new_login_fn.txt', html); // temporarily write to analyze
